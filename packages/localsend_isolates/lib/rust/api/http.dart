...
RsHttpClient  createClient({required String privateKey , required String cert , required LsHttpClientVersion version , String? expectedFingerprint , int? timeoutMs }) => RustLib.instance.api.crateApiHttpCreateClient(privateKey: privateKey, cert: cert, version: version, expectedFingerprint: expectedFingerprint, timeoutMs: timeoutMs);
...
/// Sends a small authenticated JSON command to the local Home Hub API.
 Future<String>  postJson({required ProtocolType protocol , required String ip , required int port , required String path , required String body });


 Future<PrepareUploadResult>  prepareUpload({required ProtocolType protocol , required String ip , required int port , required PrepareUploadRequestDto payload , String? publicKey , String? pin , required RsCancellationToken cancelToken });


 Future<ResultWithPublicKeyRegisterResponseDto>  register({required ProtocolType protocol , required String ip , required int port , required RegisterDto payload });
...
final PrepareUploadResponseDto? response;
        ...
        bool operator ==(Object other) =>
            ...
            other is PrepareUploadResult &&
                ...
                && statusCode == other.statusCode&& response == other.response;
...
final RegisterResponseDto body;
                ...
                const ResultWithPublicKeyRegisterResponseDto({this.publicKey ,required this.body ,});
        ...
        bool operator ==(Object other) =>
            ...
            other is ResultWithPublicKeyRegisterResponseDto &&
                runtimeType == other.runtimeType
                && publicKey == other.publicKey&& body == other.body;
...
@freezed
                sealed class RsHttpClientError with _$RsHttpClientError implements FrbException {
                    const RsHttpClientError._();
                     ...
                     const factory RsHttpClientError.statusCode({   required int status ,  String? message , }) = RsHttpClientError_StatusCode;
...
const factory RsUploadEvent.progress({   required double progress , }) = RsUploadEvent_Progress;
 /// The upload failed. Always the last event of the stream.
const factory RsUploadEvent.failed({   required RsHttpClientError error , }) = RsUploadEvent_Failed;
